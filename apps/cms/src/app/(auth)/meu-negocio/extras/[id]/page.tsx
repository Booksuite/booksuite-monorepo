...
import { useGetExtra } from "@/src/hooks/extras/useGetExtra";
import { updateExtra } from "@/src/services/extra/updateExtra";
...
export default function DetalhesExtras({ params }: { params: { id: string } }) {
  ...
  function saveExtra(e: FormEvent<HTMLFormElement>, formData: UpdateExtraDTO) {
    e.preventDefault();
  ...
  return (
    <div className="DetalhesExtras">
      ...
      {isLoading ? (
        ...
        <ExtraForm onSubmit={saveExtra} data={extra} isSaving={isSaving} />
      )}
    </div>
  );
}
